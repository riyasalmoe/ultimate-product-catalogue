...
function AddProduct($format, $Item_ID) {
		...
		if ($format == "Thumbnail") {				
				...
				$ProductString .= "</a>";	
				$ProductString .= "</div>\n";
				if ($FancyBox_Installed) {$ProductString .= "<a class='fancybox no-underline' href='#prod-cat-addt-details-" . $Product->Item_ID . "'>";}
				$ProductString .= "<div id='prod-cat-title-" . $Product->Item_ID . "' class='prod-cat-title upcp-thumb-title'>" . $Product->Item_Name . "</div>\n";
				if ($FancyBox_Installed) {$ProductString .= "</a>";}
		...
		if ($FancyBox_Installed) {
			  $uri_parts = explode('?', $_SERVER['REQUEST_URI'], 2);
				...
				$ProductString .= "<h2 class='prod-cat-addt-details-title'><a class='no-underline' href='http://" . $_SERVER['HTTP_HOST'] . $uri_parts[0] . "?Product_ID=" . $Product->Item_ID . "'>" . $Product->Item_Name . "<img class='upcp-product-url-icon' src='" . get_bloginfo('wpurl') . "/wp-content/plugins/ultimate-product-catalogue/images/insert_link.png' /></a></h2>";

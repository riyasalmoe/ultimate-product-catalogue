...
function UPCP_Add_SEO() {
	...
	if ($SEO_Option == "Yoast") {
		add_filter( 'wpseo_canonical', 'Yoast_Change_Canonical_URL', 10, 1 );
		add_filter( 'wpseo_metadesc', 'Yoast_Change_Description', 10, 1 );
		add_filter( 'wpseo_title', 'Yoast_Change_Title', 10, 1 );
		if ("Update_Breadcrumbs" == "Yes") {add_filter( 'wp_seo_get_bc_ancestors  ', 'Yoast_Change_Breadcrumbs', 10, 1 );}
	}
}

function Yoast_Change_Canonical_URL( $str ) {
...
function Yoast_Change_Title( $str ) {
	global $wpdb, $items_table_name;
...
function Yoast_Change_Breadcrumbs( $array ) {
	$SEO_Integration = get_option("UPCP_SEO_Integration");
	$Pretty_Links = get_option("UPCP_Pretty_Links");
